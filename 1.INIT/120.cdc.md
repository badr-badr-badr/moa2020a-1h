...
### a. Problème de l’utilisateur ou contexte du projet 
  ...
  De plus, nous avons constaté quelques problèmes pouvant avoir lieu au sein d'une école :
    - Difficultés pour certaines personnes de s'intégrer
    - Rencontres de gens qui ne partagent pas forcément des centres d'intérêts communs 
    - Un besoin pour certains de faire de nouvelles rencontres sans pratiquer d'activités (ces dernières fascilitant beaucoup les nouvelles rencontres)
...
### b. Acheteur
  L'application sera gratuite dans un premier temps.
...
### a. Utilisateurs directs du produit
  - Les utilisateurs directs du produit sont tous les anciens ou nouveaux élèves, les professeurs, le personnel, etc. Toute personne possèdant une adresse e-mail INSA Rouen sera autorisée à télécharger notre application.
  - Les modérateurs seront aussi des utilisateurs du produit et s'occuperont des divers signalements, par exemple des comportements déplacés. Ils auront accès à un grand nombre de fonctionnalités en plus et pourront modifier ou supprimer le profil des utilisateurs. 
...
## 4. Contraintes non négociables
  Notre application ne fonctionnera pas hors-connexion donc une connexion réseau est nécéssaire (de bonne qualité pour le chargement des données comme les photos).
### a. Contraintes sur la conception de la solution
  - L'application doit être développée dans un language adapté pour fonctionner sur plusieurs OS différents
  - L'application doit être mis à jour régulièrement et les bugs doivent être corrigés aussi vite que possible
  - L'utilisateur doit pouvoir avoir son propre compte et se connecter sur plusieurs appareils en même temps sans que cela lui nuise (par exemple sauvegarder les informations directement sur son compte et pas sur l'application elle-même)
  - Tout utilisateur ne possède qu'un seul compte mais peut se connecter sur une même application avec différents comptes
### b. Environnement de fonctionnement du système actuel
  - L'application doit pouvoir fonctionner sur plusieurs OS tels que IOS et Android.
### c. Applications « partenaires » (avec lesquelles le produit doit collaborer)
  - L'inscription se fait via notre adresse-mail INSA (@insa-rouen.fr) donc une collaboration avec l'INSA Rouen est nécessaire.
### d. « COTS » : Progiciels ou composants commerciaux
  - Il n'y a pas de composants commerciaux.
### e. Lieux de fonctionnement prévus
  Cette application peut être utilisée n'importe où et dans n'importe quel pays. L'application sera téléchargeable sur téléphone (et éventuellement plus tard via internet).
...
## 5. Glossaire et conventions de dénomination
  ...
  - Méca : Mécanique
  - Un match/Match : quand les deux personnes sont intéressés par l'autre
  - Swiper : dire que nous ne sommes pas intéressés par la personne
## 6. Faits et hypothèses utiles
### a. Facteurs influençant le produit, mais qui ne sont pas des contraintes imposées sur les exigences
  - Possibilité d'avoir une version PC dispobible.
  - Un nombre important de personnes inscrites pour que l'application fonctionne efficacement
